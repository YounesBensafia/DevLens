...
def _print_summary(report: ProjectReport):
    ...
    console.print(Columns([
        ...
        Panel(f"[white bold]{len(report.files)}[/]\n[white]Files Analyzed",
...
def _print_critical(report: ProjectReport):
    if not report.most_critical:
        return
    console.print(Panel("[bold red]Critical Files - Fix These First[/bold red]",
...
def _print_footer(report: ProjectReport):
    ...
    console.print(Panel(Align.center(Text(msg, style="bold")),
...
def run_scan_with_progress(
    ...
    avg_score = sum(r.final_score for r in file_reports) / len(file_reports) if file_reports else 0.0
    ...
    return ProjectReport(
        ...
        bus_factor_risks=[r for r in file_reports if r.git and r.git.is_orphan and r.final_score < 65],
    )
